...
3. Log in to each node individually.
...
6. Output normalized topology model and write diagram files.
...
Write the diagram to: `output/topology.drawio`
...
- Edge value: interface pair, e.g. `e1-1 | e1-49`
- Edge style: `edgeStyle=orthogonalEdgeStyle;`
- Layout: spines at y=80, leaves at y=280, clients at y=480, telemetry at y=680 or to the right

Markdown summary output: `output/topology.md`

- Must NOT contain Mermaid code blocks.
- Must state that the diagram is in `output/topology.drawio`.
- Must include: link inventory table, IP summary, per-device fact tables, discovery evidence table.
